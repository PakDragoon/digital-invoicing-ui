...
export const setFinanceInRotation = async (empId: string, companyId: string) => {
  try {
...
export const logout = async (role: string, companyId?: string) => {
  const isAdmin = role === "Admin"
  const endpoint = isAdmin ? "/admin/auth/logout" : "/employee/auth/logout"

  try {
    await api.post(endpoint, { companyId })
  } catch (error) {
...
export const getEmployee = async (id: string, companyId: string) => {
  try {
    const response = await api.get(`/employee/${id}`, { params: { companyId } })
